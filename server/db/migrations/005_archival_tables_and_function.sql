-- Migration 005 — archival tables + archive_old_data() function.
--
-- The gateway runs an archival job (careCircle.js runArchival, every 24h) that
-- calls SELECT * FROM archive_old_data(...). That function lived only in
-- server/db/archival.sql, which is NOT part of the migration chain (CI/Railway
-- bootstrap from init.sql + admin_tables.sql + run.js only), so on a clean prod
-- DB the function was absent and the job threw every cycle → no archival, and
-- unbounded growth of the time-series tables. This migration applies the same
-- idempotent DDL via the runner so it exists on every environment.
--
-- Idempotent: CREATE TABLE IF NOT EXISTS / CREATE OR REPLACE FUNCTION /
-- DROP CONSTRAINT IF EXISTS / CREATE INDEX IF NOT EXISTS. Keep in sync with
-- server/db/archival.sql.
...
BEGIN
  ...
  WITH moved AS (
    DELETE FROM health_data WHERE measured_at < v_health_cutoff RETURNING *
  )
  ...
  WITH moved AS (
    DELETE FROM medication_doses WHERE scheduled_time < v_dose_cutoff RETURNING *
  )
  ...
  WITH moved AS (
    DELETE FROM activity_logs WHERE recorded_at < v_activity_cutoff RETURNING *
  )
  ...
  WITH moved AS (
    DELETE FROM checkin_logs WHERE created_at < v_checkin_cutoff RETURNING *
  )
